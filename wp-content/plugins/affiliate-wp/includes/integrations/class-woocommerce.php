...
?>
		...
		<div id="affwp_product_variation_settings">
			...
			<p class="form-row form-row-full">
				...
				<input type="text" size="5" name="_affwp_woocommerce_variation_rates[<?php echo $variation->ID; ?>]" value="<?php echo esc_attr( affwp_format_amount( $rate ) ); ?>" class="wc_input_price" id="_affwp_woocommerce_variation_rates[<?php echo $variation->ID; ?>]" placeholder="<?php esc_attr_e( 'Referral rate (optional)', 'affiliate-wp' ); ?>" />
			</p>
		...
		if( ! empty( $_POST['variable_post_id'] ) && is_array( $_POST['variable_post_id'] ) ) {
			...
			foreach( $_POST['variable_post_id'] as $variation_id ) {
				...
				if( ! empty( $_POST['_affwp_woocommerce_variation_rates'] ) && ! empty( $_POST['_affwp_woocommerce_variation_rates'][ $variation_id ] ) ) {
					...
					$rate = affwp_sanitize_referral_rate( $_POST['_affwp_woocommerce_variation_rates'][ $variation_id ] );
