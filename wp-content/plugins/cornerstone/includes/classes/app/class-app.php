...
class Cornerstone_App extends Cornerstone_Plugin_Component {
  ...
  public function register_app_scripts( $settings, $isPreview = false ) {
    ...
    wp_register_script( 'cs-app', $app_asset['url'], array( 'wp-api-fetch', 'wp-element', 'jquery', 'lodash', 'moment', 'react', 'react-dom', 'wp-polyfill', 'wp-polyfill-fetch' ), $app_asset['version'], false );

    $v = $this->plugin->version();

    $router = $this->plugin->component( 'Router' );

    $data = apply_filters( 'cs_app_data', array_merge( array(
      'env'                       => $this->get_env(),
      'isPreview'                 => $isPreview,
    ...
    ), $isPreview ? $this->get_preview_data() : $this->get_app_data() ), $isPreview );

    wp_localize_script( 'cs-app', 'csAppData', cs_booleanize( $data ) );
  ...
  public function get_app_data() {
    ...
    return array(
      ...
      'queryBuilder'              => $this->plugin->component('Query_Builder')->get_controls(),
  ...
  public function get_preview_data() {
    return array(
      'preview' => $this->plugin->component( 'Preview_Frame_Loader' )->data()
    );
  }
  ...
  public function get_late_data() {
    return array(
      ...
      'fontData'             => apply_filters( 'cs_font_data', $this->plugin->config_group( 'common/font-data' ) ),
      'themeOptionsControls' => $this->plugin->component( 'Theme_Options' )->get_controls(),
      ...
      'elementIcons'         => $this->plugin->component( 'Element_Manager' )->get_element_icons(),
      'prefabElements'       => $this->plugin->component( 'Prefab_Elements' )->get_prefabs(),
      'defaultPresets'       => CS()->component('Template_Manager')->lookup_default_presets()
    );
  }
