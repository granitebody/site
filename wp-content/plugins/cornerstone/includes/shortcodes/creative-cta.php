...
function x_shortcode_creative_cta( $atts ) { // 1
  ...
  if ( $animation != '' ) {
    if ( $image != '' ) {
      $image_atts = cs_apply_image_atts( [ 'src' => $image_src ] );
      $graphic = '<span class="graphic"><img style="margin: 0; width: ' . $image_width . ';" src="' . isset( $image_atts['src'] ) ? $image_atts['src'] : '' . '"></span>';
    } else if ( $icon != '' ) {
