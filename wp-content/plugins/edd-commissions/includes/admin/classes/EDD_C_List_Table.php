...
class EDD_C_List_Table extends WP_List_Table {
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'payment':
				...
				return $payment ? '<a href="' . esc_url( admin_url( 'edit.php?post_type=download&page=edd-payment-history&view=view-order-details&id=' . $payment ) ) . '" title="' . __( 'View payment details', 'eddc' ) . '">#' . $payment . '</a> - ' . edd_get_payment_status( get_post( $payment ), true  ) : '&mdash;';
			case 'actions':
	...
	function process_bulk_action() {
		...
		foreach ( $ids as $id ) {
			...
			do_action( 'edd_commissions_process_bulk_action_start', $id, $current_action, $ids );
			...
			do_action( 'edd_commissions_process_bulk_action_end', $id, $current_action, $ids );
