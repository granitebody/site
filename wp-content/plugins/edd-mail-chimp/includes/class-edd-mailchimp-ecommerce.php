...
class EDD_MailChimp_Ecommerce {
			...
			if ( $default_list ) {
				...
				if( $store ) {
					...
					if( ! empty( $response['status'] ) && $response['status'] >= 400 ) {
						...
						$error_message = ! empty( $response[ 'detail' ] ) ? $response[ 'detail' ] : __( 'An default_listerror occurred while sending order details to MailChimp.', 'edd-mailchimp' );

						edd_debug_log( 'add_order() MailChimp response error: ' . $error_message );
						edd_debug_log( 'add_order() MailChimp store errors array:' . var_export( $response['errors'],true ) );
					...
					edd_debug_log( 'add_order() payment ' . $payment_id . ': store could not be retrieved for list ' . $default_list->remote_id );
				...
				if ( ! empty( $preferences ) ) {
					foreach( $preferences as $list ) {
						...
						if( $store ) {
							...
							edd_debug_log( 'add_order() payment ' . $payment_id . ': store for ' . $list->remote_id . ' not found' );
	...
	public function refund_order( $payment) {
		...
		try {
				...
				if ( ! empty( $preferences ) ) {
					...
					foreach( $preferences as $list ) {
						...
						if( ! $store->api->success() ) {
							...
							edd_debug_log( 'refund_order() payment ' . $payment->ID . ' updated on default list successfully' );
			...
			edd_insert_payment_note( $payment->ID, __( 'Order details have been updated in MailChimp.', 'eddmc' ) );
			return true;

		} catch (Exception $e) {
			edd_debug_log( 'refund_order(): Exception encountered while updating ' . $payment->ID . ' on list' );
			edd_insert_payment_note( $payment->ID, __( 'MailChimp Ecommerce360 Error: ', 'eddmc' ) . $e->getMessage() );
			return false;
		}
	...
	public function cancel_order( $payment_id, $new_status, $old_status ) {
		...
		edd_debug_log( 'cancel_order() processing for ' . $payment_id );
		...
		try {
			...
			if ( $default_list ) {
				...
				if( ! $store->api->success() ) {
					...
					edd_debug_log( 'cancel_order() payment ' . $payment_id . ' updated successfully' );
