...
class EDD_MailChimp_Metabox {
	...
	public function render_metabox() {
		...
		if ( ! empty( $connected_lists ) ) {
			?>
			<p>
				<?php _e( 'Select the lists you wish buyers to be subscribed to when purchasing.', 'eddmc' ); ?>
				<span alt="f223" class="edd-help-tip dashicons dashicons-editor-help"
					  title="<strong><?php printf( __( '%s Specific Lists', 'eddmc' ), edd_get_label_singular() ); ?>:</strong> <?php _e( 'Lists and groups selected here will be subscribed to upon checkout, regardless of the opt-in status of the customer.', 'eddmc' ); ?>"></span>
			</p>
			<?php
		...
		echo '<label>';
		$global_double_opt_in = edd_get_option( 'eddmc_double_opt_in', false );

		if ( false === $global_double_opt_in ) {

			// If global double opt-in is disabled, allow specific products to use double opt-in.
			...
			// If global double opt-in is enabled, specific products must honor this and cannot bypass it.
			echo '<input type="checkbox" name="edd_mailchimp_double_opt_in" value="1"' . checked( true, true, false ) .' readonly disabled>&nbsp;';
		...
		// Display a tooltip to inform the user when this setting is disabled due to the global configuration.
		if ( $global_double_opt_in ) {
			?>
			<span
				alt="f223"
				class="edd-help-tip dashicons dashicons-editor-help"
				title="<strong><?php _e( 'Double Opt-In', 'eddmc' ); ?></strong>: <?php _e( 'MailChimp for Easy Digital Downloads is currently configured to globally require Double Opt-In. To manage this at the product level, you will need to disable the global Double Opt-In requirement.', 'eddmc' ); ?>"></span>
			<?php
		}
	}
