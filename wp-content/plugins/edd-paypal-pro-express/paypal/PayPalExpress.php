	...
	public function retrieve_token() {
		...
		if ( is_array( $cart_details ) && ! empty( $cart_details ) ) {
			...
			foreach ( $cart_details as $item ) {
				...
				$description .= stripslashes_deep( html_entity_decode( edd_get_cart_item_name( $item ), ENT_COMPAT, 'UTF-8' ) ) . ', ';
				$item_data   = array(
					'name'     => stripslashes_deep( html_entity_decode( edd_get_cart_item_name( $item ), ENT_COMPAT, 'UTF-8' ) ),
					'amount'   => $item_amount,
					'number'   => edd_use_skus() ? edd_get_download_sku( $item['id'] ) : $item['item_number']['id'],
			...
			foreach ( $this->_purchase_data['fees'] as $fee ) {
				...
				if ( empty( $fee['download_id'] ) && floatval( $fee['amount'] ) > '0' ) {
					...
					$item_data   = array(
						...
						'amount'   => edd_sanitize_amount( $fee['amount'] ),
						'quantity' => 1
					);
		...
		if ( ( $item_total + $tax ) > $total_amount ) {
			...
			$item_data = array(
				...
				'amount' => edd_sanitize_amount( 0 - $discount ),
				'number' => $this->_purchase_data['discount'],
				'quantity' => 1
			);
			$items[] = edd_sanitize_amount( 0 - $discount );
		...
		$ppfunctions->request_fields( apply_filters( 'eppe_get_express_paypal_data', $paypal_data, $this->_purchase_data ) );
