...
function eddpdfi_pdf_template_blue_stripe( $eddpdfi_pdf, $eddpdfi_payment, $eddpdfi_payment_meta, $eddpdfi_buyer_info, $eddpdfi_payment_gateway, $eddpdfi_payment_method, $address_line_2_line_height, $company_name, $eddpdfi_payment_date, $eddpdfi_payment_status ) {
	...
	$eddpdfi_payment  = edd_get_payment( $eddpdfi_payment->ID );
	$payment_meta     = $eddpdfi_payment->get_meta();
	$payment_currency = $eddpdfi_payment->currency;
	$cart_items       = $eddpdfi_payment->cart_details;
	$customer_id      = $eddpdfi_payment->customer_id;
	...
	$line_height_totals = 0;
	if ( ! empty( $edd_options['eddpdfi_name'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_name'] );
	}
	if ( ! empty( $edd_options['eddpdfi_address_line1'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_address_line1'] );
	}
	if ( ! empty( $edd_options['eddpdfi_address_line2'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_address_line2'] );
	}
	if ( ! empty( $edd_options['eddpdfi_address_city_state_zip'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_address_city_state_zip'] );
	}
	if ( ! empty( $edd_options['eddpdfi_address_country'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_address_country'] );
	}
	if ( ! empty( $edd_options['eddpdfi_email_address'] ) ) {
		$line_height_totals += eddpdfi_calculate_line_height( $edd_options['eddpdfi_email_address'] );
	}
...
$eddpdfi_pdf->Cell( 0, 6, $country, 0, 2, 'L', false );
	...
	if ( $eddpdfi_pdf_downloads ) {
		...
		foreach ( $eddpdfi_pdf_downloads as $key => $cart_item ) {
			...
			$user_id      = $eddpdfi_payment->user_id;
			$payment_date = strtotime( $eddpdfi_payment->date );
			...
			$eddpdfi_total_price = edd_currency_filter( edd_format_amount( $eddpdfi_payment->total ), $payment_currency );
		...
		if ( edd_use_taxes() ) {
			$eddpdfi_pdf->SetX( 35 );
			$eddpdfi_pdf->Cell( 102, 8, eddpdfi_get_settings( $eddpdfi_pdf, 'tax_label' ) . ' - ' . $tax, 0, 2, 'L', false );
		}
		...
		$total = html_entity_decode( edd_currency_filter( edd_format_amount( $eddpdfi_payment->total ), $payment_currency ), ENT_COMPAT, 'UTF-8'  );
