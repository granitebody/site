...
$eddpdfi_pdf->Cell( 0, 6, $country, 0, 2, 'L', false );
	...
	if ( $eddpdfi_pdf_downloads ) {
		...
		if ( edd_use_taxes() ) {
			$eddpdfi_pdf->SetX( 35 );
			$eddpdfi_pdf->Cell( 0, 11, eddpdfi_get_settings( $eddpdfi_pdf, 'tax_label' ) . ' - ' . $tax, 0, 2, 'R', false );
		}
		...
		if ( ! empty ( $fees ) ) {
			foreach( $fees as $fee ) {
				$eddpdfi_pdf->SetX( 35 );
				$eddpdfi_pdf->Cell( 0, 8, $fee['label'] . ' - ' . html_entity_decode( edd_currency_filter( $fee['amount'], $payment_currency ), ENT_COMPAT, 'UTF-8' ), 0, 2, 'R', false );
			}
		}
