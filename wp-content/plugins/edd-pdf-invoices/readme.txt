...
Plugin URI: https://easydigitaldownloads.com/downloads/pdf-invoices/
...
= 2.2.27 =
* Fix: Updated TCPDF library for compatibility with PHP 7.4.
* Tweak: Update references to `EDD_Payment` to `edd_get_payment()` in preparation for EDD 3.0 compatibility.
* Tweak: Update plugin author name and URL to Sandhills Development.

= 2.2.26 =
* FIx: Compatibility with PHP 7.3

= 2.2.25 =
* Fix: Incorrect version comparison check for EDD core prevented PDF Invoices from fully loading.

= 2.2.24 =
* Fix: PDF Invoices was producing a fatal error when activated without Easy Digital Downloads being active.
* Fix: The default template would cause the store email and domain to overlap with the customer data if they were too long.
* Fix: There was a missing closing anchor tag in the Download Invoice link.

= 2.2.23 =
* New: Added country selector the PDF Invoices settings and the country is now included on the generated invoice.
* New: Added settings to change the Tax label and invoice heading.
* Fix: Added a comma between the customer city and state.
* Fix: Corrected an issue in the Traditional invoice template that caused overlaps to occur.
* Fix: Updated all invoice templates to use the Payment's currency, instead of the store's.

= 2.2.22 =

* Fix: Payment currency not respected; default shown instead.
